...
class Preprocessor:
    ...
    def __init__(self, path_to_stopwords, language: str = "pl"):
        self.language = language
        if self.language == "pl":
            # load Morfeusz
            self.morfeusz = morfeusz2.Morfeusz
            self.stopwords_file_path: str = path_to_stopwords
            # load stopwords from nltk
            self.stopwords = set(get_stop_words("polish"))

            # load stopwords from txt and union with stopwords
            self.stopwords |= set(
                open(self.stopwords_file_path, "r", encoding="utf-8")
                .read()
                .splitlines()
            )
    ...
    def remove_punctuation(self, cols: list):
        ...
        for col in cols:
            self.dataset[col] = self.dataset[col].str.translate(
